...
export const updateApprovedSource = (
  mongo_db,
  mongo_collection,
  mongo_uri,
  path,
  ...
  MongoClient.connect(mongo_uri, { useNewUrlParser: true }, (err, client) => {
    ...
    readFile(path, "utf8", async (err, data) => {
      ...
      for (let row of recordsWithoutHeader) {
        ...
        let approved_source = row[1].trim();
        ...
        updateArray.push({
          updateOne: {
            ...
            update: {
              $set: {
                "account.approved_source": approved_source,
      ...
      for (let i = 0; i < n; i++) {
        ...
        await collection
          ...
          .then((res) => {
            ...
            console.log(
              `Chunk ${i + 1}/${n} complete. Matched: ${res.matchedCount}, Modified: ${res.modifiedCount}. Total modified so far: ${updatedDocuments}`,
            );
          })
          .catch((err) => {
      ...
      console.log(
        `Migration finished. Total documents modified: ${updatedDocuments}`,
      );
      client.close();
    });
  });
